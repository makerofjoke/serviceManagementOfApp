...
<form id="form" method="post" action=""  prevURL="" class="form-horizontal required-validate" onsubmit="return validateCallback(this,getSaverslt);">
...
<%-- 	<input type="hidden" id="relativeId" name="relativeId" value="${relativeId}"> --%>
	<div class="" style="padding: 0.4% 1.1% 0.1% 0.1%">
		    	<label class=""  style="margin-left:-7%;font-size:20px;text-align:right;color:#FFFFFF" >容器端口设置</label>
			</div>
		<div class="form-group" style="padding: 0.4% 1.1% 0.1% 0.5%">
		    ...
		    <div class="col-xs-3" style="margin-left: -6.7%;">
		    	 <input type="text" name="portName" class="form-control" value="${portName}" onkeyup="javascript:checkName('portName');" style="width: 100%;color:#c0c0c0;background: rgba(216,216,216,0.30);border-radius: 10px;" required="required"/>
			</div>
		</div>
		<div class="form-group" style="padding: 0.4% 1.1% 0.1% 0.5%">
			<label for="dataIP" class="control-label col-xs-2"  style="display:;text-align:right;"><font class="white">&emsp;端口号：</font></label>
		    <div class="col-xs-3" style="margin-left: -6.7%;">
		    	 <input type="text" name="portNumber" class="form-control" value="${portNumber}" onkeyup="javascript:checkInt('portNumber');"  style="width: 100%;color:#c0c0c0;background: rgba(216,216,216,0.30);border-radius: 10px;" required="required"/>
			</div>
		</div>
		<!-- 
...
<script type="text/javascript">
 ...
 function getSaverslt(){
	 ...
	 var portName = $("input[name='portName']").val();
	 var portNumber = $("input[name='portNumber']").val();
	 var doc = findLayDoc();
	 var ports = doc.find("#tContainerPort");
 			var tr = "<tr target='slt_uid' rel='' id='"+id+"'> "
 				+" <td align='center' valign='middle'><font class='white'>"+portName+"</font></td>"
 				+" <td align='center' valign='middle'><font class='white'>"+portNumber+"</font></td>"
 				...
 				+"	            	<li><a class='layOpenWindow'  href=javascript:deleteContainerPort('"+id+"') >删除</a></li>"
 				+"	            </ul>"
 				+"          </div> "
 				+"		</td>"
 				+"  </tr>";
